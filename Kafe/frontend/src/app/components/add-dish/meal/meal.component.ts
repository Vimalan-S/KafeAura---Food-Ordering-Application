...
export class MealComponent {
  mealForm: FormGroup;
  dishCategory: string = 'meal'; // Fixed as 'meal' for this component
  ...
  get images() {
    return this.mealForm.get('images') as any;
  }
  ...
  submitForm() {
    if (this.mealForm.valid) {
      ...
      this.http.post('http://localhost:3000/admin/add-dish/meals', formData)
        .subscribe({
          next: (response) => {
  ...
  private initializeAvailableSlots() {
    ...
    this.mealForm.setControl('availableSlot', availableSlotFormArray);
  }
